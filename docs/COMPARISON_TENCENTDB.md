...
| 优先级 | 改进项 | 工作量 | 收益 |
|--------|--------|--------|------|
| P0 | 分层注入策略（摘要+索引模式） | 小（1-2天） | 高：Token 效率显著提升 |
| P1 | L0-L3 分层标记 + 提炼管线 | 中（1周） | 高：记忆质量系统性提升 |
| P2 | Skill 结构化字段扩展 | 小（2-3天） | 中：Skill 复用体验提升 |
| P3 | Extraction 闭环（Proxy 回复提取） | 中（1周） | 中：自动化沉淀能力 |
| P4 | Team Schema 预留 | 小（1天） | 低（远期铺垫） |
