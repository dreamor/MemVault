...
| **D 可选（已落地）** | 团队共享池（`visibility=shared`，跨命名空间注入上限 20）；SOP/Markdown 批量导入（`sop.rs` + CLI `import-skills` + MCP `import_skills`）；Obsidian 分类型目录同步（`folderFor`） | 全 workspace 测试绿（716 passed / 0 failed，2026-09-01） |
...
| **双轨注入（压缩态 + 逐字检索）** | 注入预算拆成两条轨道：常驻的固定大小"用户/项目状态摘要"（增量维护）+ 按需检索的记忆全文。依据：Qwen3.8-Flash-Next 技术报告 §2.1.1 证明压缩态与逐字检索缺一不可。涉及新记忆形态（聚合摘要生成与维护管道），超出当期范围 | 未启动（远期规划） |
...
| **REST `/api/memories/{id}/evidence` 端点** | 为 Dashboard 证据图谱提供按记忆查证据/被证关系的 REST 出口；MCP 侧读写已齐（`add_evidence` 写、`get_memory_evidence` 读——内部为 `trace_evidence_chain` + `evidence_summary`，已有测试覆盖），REST 缺的只是薄封装 | 已实现（2026-09-10，随 dashboard 详情面板证据链展示落地）|
...
> 对照分析 `docs/PAPER-INSPIRATIONS.md` 已完成落地并**归档删除**，落地项详见 CHANGELOG。Qwen3.8-Flash-Next 技术报告的三个记忆相关组件（§2.1.1 GDN 压缩态 + 周期性全注意力、§2.1.2 QSA 两级稀疏检索、§2.3 N-gram 条件记忆）与 MemVault 同构，对照分析后落地 5 项、暂缓 1 项。论文方法论一并沿用：每个改动沿**质量 / 成本 / 稳定性**三轴评测，并记录负面结果。
...
> 文档更新至 **2026-08-31**：Phase 1–10 已全部交付。后续演进（如团队共享隔离、SOP 导入、情景/程序/语义三类记忆闭环）已落地并有验收证据，见 §15、§16 与 `docs/experiments/`。