...
> **版本**：v1.0
> **日期**：2026-08-27（v0.1 起草于 2026-08-26）
> **状态**：✅ 已实施——Phase A / B / C / D 全部落地，H5 / H6 / H7 验收实验均 CONFIRMED；仅「图数据库集成」按计划保持远期推迟
...
> 这 5 个问题在 Phase A–D 实现过程中均已落地为具体设计；「结论」列为最终采纳方案及落点。

| # | 问题 | 结论 | 落点 |
|---|---|---|---|
| Q1 | 教训升级为 MUST 是否需要人工确认？ | **需要**（防止低置信教训变强制指令）——系统仅产出升级**提示**，从不自动升 MUST | `reflection.rs::escalation_hint`（suggest-only） |
| Q2 | `episodes` 与 `memories` 一对一还是一对多？ | **一对一**（保持检索统一）；重试场景用多条 episode | `episodes.memory_id` 为 PK（migration 6） |
| Q3 | 技能成功率展示的最小样本数？ | **3 次**（低于则不展示百分比） | `models.rs::SKILL_RATE_MIN_SAMPLES = 3` |
| Q4 | 关系抽取是否默认开启？ | **默认关闭**，`MEMVAULT_RELATIONS=on` 显式开启（成本与噪音考量） | `server.rs` 关系抽取门控 |
| Q5 | 教训是否需要跨命名空间共享（全局避坑）？ | **默认命名空间内**；跨项目共享改由显式机制承载——`visibility='shared'`（Phase D 团队共享池） | `visibility` 列（migration 14）+ `list_shared` 注入 |
