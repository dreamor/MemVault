...
### Docs
- **README 双语对齐与纠错**：EN/ZH 两版 README 逐行核对仓库实现并修正——测试数量由 `~970` 更新为 `~1000`（`cargo test --all-features -- --list` 实测 1001）；补齐 `.env` 配置表缺失的三个真实读取键 `MEMVAULT_HOME` / `HF_ENDPOINT` / `MEMVAULT_EXTRACT_ASSISTANT`；新增缺失的「项目状态 / Project Status」章节，修复 TOC 与 Status badge 的死链锚点；两阶段注入说明移除无法溯源的具体模型引用，改为「两阶段设计」；`integrations/codex/README.md` 的 MCP 工具数由 16 同步为 18；根 `plugin.json` 版本回拨至与 workspace 一致的 `0.3.0`。
...
### Removed
- **GUI 面层收敛：删除 VS Code 扩展，Obsidian 插件瘦身为「Vault 同步 + 查看/搜索/选区捕获」**：`vscode-extension/` 整体移除（含 CI `vscode-extension` / `vscode-package` / `open-vsx` jobs、CODEOWNERS、bug 模板、CONTRIBUTING、README 双语与 INSTALL / RELEASING / DISTRIBUTION 文档引用同步清理）；Obsidian 插件删除与 Web Dashboard / CLI 重复的管理面（Review Inbox / approve / reject / quick-edit、Supersede、Edit、Stats、Export / Import / Backup、Checkpoints、dedup / decay / promote），保留 Vault 双向同步、浏览 / 搜索、选区保存 / 提取、mark-read、Delete；插件已重新构建，vitest 31 例通过。
...
### Changed
- **dsh 插件包名更名**:`@memvault/dsh-plugin` → `@dreamor/dsh-memvault`——dsh 设置→插件→插件列表的显示名由 `plugin` 变为 `memvault`(列表名由 npm 包名派生);MCP 握手 `clientInfo.name` 同步改为 `memvault-dsh-memvault`。已安装的 profile 需重新 `pnpm install`(bundles 已同步更新),新装请用新包名。
