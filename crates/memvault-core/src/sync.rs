...
pub struct SyncConfig {
    ...
    /// Lightweight catalog (type + priority + content-derived one-liner, no
    /// full text) — a cheap "scan the map first" artifact, distinct from the
    /// full-content generators above. See docs/PERSONAL-MEMORY-INSPIRATION.md.
...
impl SyncEngine {
    ...
    pub async fn sync(&self, project_dir: &Path) -> Result<SyncReport> {
        ...
        if self.config.generate_index {
            let content = self.generate_index_md(&memories);
            let path = project_dir.join("MEMORY-INDEX.md");
            self.write_if_changed(&path, &content)?;
            report.files_written.push(path);
        }
        skip_if_disabled(self.config.generate_index, "MEMORY-INDEX.md", &mut report);
    ...
    /// Lightweight catalog: one line per memory (type + priority + a
    /// content-derived pseudo-title), no full instruction/content text —
    /// meant as a cheap "scan the map first" artifact a human or low-budget
    /// agent can read before pulling full content via CLAUDE.md/AGENTS.md or
    /// `search_memory`. MemVault has no dedicated title/description field
    /// yet, so v1 derives the pseudo-title from `content`'s first ~80 chars
    /// (deliberate simplification, see docs/PERSONAL-MEMORY-INSPIRATION.md).
    fn generate_index_md(&self, memories: &[SearchResult]) -> String {
        let mut out = String::from("# Memory Index (Auto-generated by MemVault)\n\n");
        out.push_str(
            "> 一行一条，只列类型/优先级/内容摘要，不含正文；完整内容见 CLAUDE.md/AGENTS.md 或 search_memory。\n\n",
        );

        for r in memories {
            let mem = &r.memory;
            let type_str = serde_json::to_string(&mem.memory_type).unwrap_or_default();
            let type_str = type_str.trim_matches('"').to_uppercase();
            let priority_str = match mem.priority {
                Priority::Must => "MUST",
                Priority::Reference => "REFERENCE",
                Priority::Background => "BACKGROUND",
            };
            let char_count = mem.content.chars().count();
            let title: String = mem.content.chars().take(80).collect();
            let title = if char_count > 80 {
                format!("{title}...")
            } else {
                title
            };
            ...
            out.push_str(&format!(
                "- [{type_str}][{priority_str}] {title} (namespace: {}{tags})\n",
                mem.namespace
            ));
        }
...
mod tests {
    ...
    #[test]
    fn test_generate_index_md_one_line_per_memory_no_full_text() {
        let store = Arc::new(crate::storage::sqlite::SqliteStore::in_memory().unwrap());
        let engine = SyncEngine::new(store);
        let agent = SourceAgent {
            id: "t".into(),
        ...
        let mut must = mock_result("always use Python, never Java", Priority::Must, &agent);
        must.memory.memory_type = MemoryType::Preference;
        must.memory.instruction = Some("full instruction text that would bloat an index".into());

        let mut refm = mock_result("project uses FastAPI", Priority::Reference, &agent);
        refm.memory.memory_type = MemoryType::Fact;
        refm.memory.tags = vec!["coding".into(), "python".into()];

        let content = engine.generate_index_md(&[must, refm]);

        assert!(content.contains("[PREFERENCE][MUST]"));
        assert!(content.contains("always use Python, never Java"));
        // Index is a catalog, not the full-text generators — instruction
        // text must never leak in, only the content-derived pseudo-title.
        assert!(!content.contains("full instruction text that would bloat an index"));
        assert!(content.contains("[FACT][REFERENCE]"));
        assert!(content.contains("tags: coding,python"));
    }

    #[test]
    fn test_generate_index_md_must_first_ordering_preserved() {
        // select_memories already sorts MUST-first; the generator must not
        // re-sort or reorder — it just renders whatever order it's given.
        let store = Arc::new(crate::storage::sqlite::SqliteStore::in_memory().unwrap());
        let engine = SyncEngine::new(store);
        let agent = SourceAgent {
            id: "t".into(),
        ...
        let must = mock_result("must rule", Priority::Must, &agent);
        let refm = mock_result("ref context", Priority::Reference, &agent);

        let content = engine.generate_index_md(&[must, refm]);
        let must_pos = content.find("must rule").unwrap();
        let ref_pos = content.find("ref context").unwrap();
        assert!(must_pos < ref_pos);
    }

    #[test]
    fn test_generate_index_md_truncates_long_content() {
        let store = Arc::new(crate::storage::sqlite::SqliteStore::in_memory().unwrap());
        let engine = SyncEngine::new(store);
        let agent = SourceAgent {
            id: "t".into(),
        ...
        let long = mock_result(&"x".repeat(200), Priority::Reference, &agent);
        let content = engine.generate_index_md(&[long]);
        assert!(content.contains("..."));
    }

    #[tokio::test]
    async fn test_sync_writes_memory_index_file() {
        let store = setup().await;
        let engine = SyncEngine::new(store);

        let dir = temp_dir("index_write");
        let report = engine.sync(&dir).await.unwrap();
        ...
        assert!(
            report
                .files_written
                .iter()
                .any(|p| p.ends_with("MEMORY-INDEX.md"))
        );
        let content = std::fs::read_to_string(dir.join("MEMORY-INDEX.md")).unwrap();
        assert!(content.contains("Memory Index"));
        assert!(!content.contains("Always use Python, never Java")); // that's the instruction, not content
        std::fs::remove_dir_all(&dir).ok();
    }
}
